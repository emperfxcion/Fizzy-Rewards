...
export default function StaffClient() {
  ...
  const allow = useMemo(() => {
    ...
    return Boolean(ALLOWLIST[email]) || me?.role === 'staff' || me?.role === 'owner';
  }, [me, meEmail]);
  ...
  async function init() {
    ...
    if (allowRole) {
      await supabase.from('profiles').upsert({ id: user.id, role: allowRole, full_name: null, phone: null });
  ...
  async function runSearch() {
    ...
    const { data: loy } = await supabase
      .from('loyalty').select('user_id, stamps').in('user_id', ids);
  ...
  async function giveStamp(target: Profile, delta = 1) {
    ...
    if (user) {
      await supabase.from('stamp_events').insert({
        user_id: target.id, staff_id: user.id, delta, note: 'stamp given'
      });
    }

    const { data: l } = await supabase
      .from('loyalty').select('user_id, stamps').eq('user_id', target.id).maybeSingle();
  ...
  async function redeemByCode() {
    ...
    const { data: red, error } = await supabase
      .from('redemptions').select('id, user_id, status').eq('code', c).maybeSingle();
    if (error) { alert(error.message); return; }
    ...
    const { data: l2 } = await supabase
      .from('loyalty').select('user_id, stamps').eq('user_id', red.user_id).maybeSingle();
  ...
  return (
    <div className="space-y-4">
      <div className="card">
        ...
        <p className="opacity-80">Signed in as: {me?.full_name || emailShown} ({roleShown})</p>
      ...
      {allow ? (
        <>
          <div className="card space-y-3">
            ...
            <div className="flex gap-2">
              <input className="flex-1 rounded-2xl border px-4 py-2"
        ...
        <div className="card bg-yellow-50 border">
