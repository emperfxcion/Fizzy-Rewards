...
export default function Dashboard() {
  ...
  async function init() {
    ...
    if (!profileRow) {
      ...
      await supabase
        .from('profiles')
        .insert({ id: user.id, full_name: name, phone, role: 'customer' })
  ...
  async function redeem() {
    ...
    const { error } = await supabase
      .from('redemptions')
      .insert({ user_id: profile!.id, code: candidate, status: 'issued' })
    if (!error) {
      setCode(candidate)
      setConfetti(true)
    ...
    <div className="space-y-4">
      ...
      <div className="card">
        ...
        <p className="opacity-80">
          Show this screen when ordering to earn stamps.
        </p>
        ...
        <div className="flex items-center gap-6 mt-4">
          ...
          <div>
            <p className="text-lg font-semibold">
              {loyalty?.stamps ?? 0} / {REWARD_THRESHOLD} stamps
            </p>
            <p className="opacity-70 text-sm">10 stamps = 1 free drink</p>

            <div className="mt-4 flex gap-2">
              <a className="btn" href="/staff" style={{ background: '#3EC9F5' }}>
                Staff
              </a>
              <button
                className="btn"
                onClick={redeem}
                disabled={!loyalty || loyalty.stamps < REWARD_THRESHOLD}
              >
                Redeem free drink
              </button>
            </div>
            ...
            {code && (
              <div className="mt-4 p-3 rounded-2xl bg-brand-blush/60 border">
                ...
                <p className="text-sm opacity-80">
                  Show this to staff to claim your free drink.
                </p>
              </div>
            )}
          </div>
